...
TEST_TEMP_ROOT = REPO_ROOT / ".codex" / "tmp" / "packet-workflow" / "git-split-and-commit" / "integration-tests"
...
class GitSplitAndCommitIntegrationTests(unittest.TestCase):
    ...
    def test_apply_persists_footer_in_created_commit_message(self) -> None:
        tmp = self.workdir("footer")
        repo = tmp / "repo"
        artifacts = tmp / "artifacts"
        repo.mkdir()
        artifacts.mkdir()
        ...
        final_message = run_command(["git", "show", "-s", "--format=%B", "HEAD"], cwd=repo).stdout
        self.assertEqual(
            final_message,
            "fix(core): update app output\n\n- update the tracked runtime file\n\nRefs: #22\n\n",
        )
